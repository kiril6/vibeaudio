...
- Playback backend is detected once from `PLAYER_CANDIDATES` (`afplay` → `paplay` → `ffplay` → `aplay`, plus PowerShell `SoundPlayer` on Windows). `aplay` and PowerShell can't attenuate, so `--volume` is a no-op there. If nothing is found, it warns once and runs silently — audio must never block the wrapped command.
