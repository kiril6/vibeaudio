...
`--uninstall-hooks` also calls `stopDaemon()` — from `cli.js`, not from `uninstallHooks()`, which stays a pure settings edit so tests can call it against a throwaway file without touching the real `~/.vibeaudio/daemon.pid`. Without that stop, removing the hooks orphans a running player: nothing will send `Stop` again, and `npm rm -g` right after takes away the only binary that could kill it.
...
`promptInteractive()` returns intent (`installHooks`, `reactive`) and never writes config itself — `cli.js#installHooksFromMenu` performs the install so both entry points share one code path and one report. The delivery and reactive questions are asked only for `claude`, the one tool VibeAudio installs hooks for; asking elsewhere would offer a control that does nothing. Codex (`~/.codex/hooks.json`, same entry shape, events `user_prompt_submit` / `pre_tool_use` / `stop`) and Cursor (`~/.cursor/hooks.json`, `{version: 1, hooks: {beforeSubmitPrompt: [{command, timeout}], stop: [...], preToolUse: [...]}}`) both have equivalent systems we do not write to yet — so "hooks are a Claude Code feature" is a statement about this project, not about the ecosystem. An install that throws (npx checkout, malformed settings) is reported and the tool still launches — the user came to start an agent, not to configure one.
